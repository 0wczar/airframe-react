...
export class Row extends React.Component {
    ...
    componentDidUpdate(nextProps) {
        if (!_.isEqual(nextProps.gridSize, this.props.gridSize)) {
            if (!_.isEmpty(this._lastLayouts)) {
                this._updateTrueColSizes(
                    this._lastLayouts[this.state.activeLayout],
                );
            }
        }
    }
    ...
    render() {
        const { children, rowHeight, onLayoutChange, ...otherProps } =
            this.props;
        const { trueColSizes } = this.state;

        if (this.context.active) {
            const layouts = (this._lastLayouts =
                this._calculateLayouts(children));
            ...
            return (
                <ResponsiveGrid
                    ...
                    onLayoutChange={(currentLayout, allLayouts) => {
                        // Notify the parent
                        onLayoutChange(
                            this._transformForChangeHandler(allLayouts),
                        );
                    ...
                    onResize={(layout, oldItem, newItem) => {
                        this.setState({
                            trueColSizes: {
                                ...trueColSizes,
                                [newItem.i]: this.context.gridUnitsToPx(
                                    newItem.w,
                                    newItem.h,
                                ),
                            },
    ...
    _updateTrueColSizes = (layout) => {
        ...
        for (let child of layout) {
            trueColSizes[child.i] = this.context.gridUnitsToPx(
                child.w,
                child.h,
            );
        }
